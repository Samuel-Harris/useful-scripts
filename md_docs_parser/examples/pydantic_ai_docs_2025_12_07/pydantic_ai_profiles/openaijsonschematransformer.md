...
See https://platform.openai.com/docs/guides/function-calling?api-mode=responses#strict-mode for more details, but this basically just requires: _ `additionalProperties` must be set to false for each object in the parameters _ all fields in properties must be marked as required
